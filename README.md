--[[
    ...
    Insta Reset com múltiplos métodos anti‑kick.
]]
...
local _GuiKeys = nil
local instaResetEnabled = true  -- <-- toggle para desabilitar o insta reset
...
-- ============================================================
-- INSTA RESET MELHORADO (3 métodos + delays aleatórios)
-- ============================================================
cursedInstaReset = function()
    if not instaResetEnabled then return end  -- toggle desligado
    ...
    if humanoid and humanoid.Health <= 0 then
        -- já está morto, tenta resetar pelo remote
    ...
    local conns = {}
    if humanoid then
        table.insert(conns, humanoid.Died:Connect(function() resetDone = true end))
    end
    if character then
        table.insert(conns, character.AncestryChanged:Connect(function(_, parent)
            if not parent then resetDone = true end
        end))
    end

    -- Método 1: remote event
    task.spawn(function()
        for i = 1, 8 do
            if resetDone then break end
            pcall(function()
                if cursedResetRemote then
                    cursedResetRemote:FireServer(CURSED_RESET_GUID, LP, "balloon")
                end
            end)
            task.wait(math.random(5, 15) / 10) -- 0.5~1.5s aleatório
        end
    end)

    -- Método 2: matar o humanoid (se ainda não resetou)
    task.delay(0.8, function()
        if not resetDone and humanoid and humanoid.Health > 0 then
            pcall(function()
                humanoid.Health = 0
                humanoid:BreakJoints()
            end)
            task.wait(0.3)
            if not resetDone and character then
                pcall(function()
    ...
    -- Método 3: simular tecla R e forçar respawn via CoreGui (fallback)
    task.delay(1.5, function()
        if not resetDone then
            pcall(function()
                -- simula a tecla R (respawn)
                ...
                -- força respawn pelo PlayerScripts se existir
                local ps = LP:FindFirstChild("PlayerScripts")
                if ps then
                    local respawn = ps:FindFirstChild("Respawn")
                    if respawn then
                        respawn:FireServer()
                    end
                end
    ...
    -- limpeza das conexões após 3 segundos
...
pcall(function()
    ...
    if type(d.instaResetEnabled)=="boolean" then instaResetEnabled=d.instaResetEnabled end
end)
...
;(function()
    ...
    lockBtn.Position = UDim2.new(1, -70, 0, 8)  -- ao lado do botão de minimizar
...
;(function()
    ...
    -- Toggle para desabilitar o Insta Reset (caso continue dando kick)
    local _,svInstaReset = addToggleRow(cp,"Insta Reset Enabled",instaResetEnabled,18,nil,function(on)
        instaResetEnabled = on
        saveConfig()
    end)
end)()
...
;(function()
    ...
    -- ===== LOCK UI TOGGLE (já existe na barra superior, mas mantemos o toggle na GUI para controle) =====
    addToggleRow(vi,"Lock UI (freeze drag)",uiLocked,16,nil,function(on)
        uiLocked = on
        -- Atualiza o botão da barra superior também
        if lockBtn then
            lockBtn.Text = uiLocked and "🔒" or "🔓"
            lockBtn.BackgroundColor3 = uiLocked and Color3.fromRGB(60,200,80) or Color3.fromRGB(50,50,50)
        end
...
print("ASTA REMAKE LOADED – Speed Booster integrado, botão de Lock na barra, Insta Reset reforçado!")
